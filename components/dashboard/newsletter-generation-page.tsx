/** biome-ignore-all assist/source/organizeImports: // biome-ignore lint: false positive */
"use client";
...
import { NewsletterLoadingCard } from "./newsletter-loading-card";
import { NewsletterDisplay } from "./newsletter-display";
// biome-ignore lint/style/useImportType: // biome-ignore lint: false positive
import { NewsletterObject, NewsletterSchema } from "@/lib/newsletter/types";
import z from "zod";
...
export function NewsletterGenerationPage() {
  ...
  useEffect(() => {
    ...
    const startGeneration = async () => {
      try {
        // Get metadata for toast notification
        ...
        console.error("Failed top prepare newsletter:", error);
        // Start generation anyways
        submit(params);
      }
    };
    startGeneration();
  ...
  const handleSave = async () => {
    ...
    try {
      await saveGeneratedNewsletter({
        newsletter: newsletter as NewsletterObject,
  ...
  return (
    <div className="bg-linear-to-r from white to-gray-50 dark:from-black dark:to-gray-950 min-h-screen">
      <div className="container mx-auto py-12 px-6 lg:px-8 space-y-8">
        ...
        <div className="flex items-center justify-between">
          ...
          {isLoading && (
            <div className="flex ites-center gap-2 text-base">
              <div className="inline-flex size-8 items-center justify-center rounded-md bg-linear-to-r from-cyan-500 to-teal-600 text-white animate-pulse">
        ...
        {!isLoading && !newsletter?.body && (
          <Card className="transition-al hover:shadow-lg">
            <CardHeader>
