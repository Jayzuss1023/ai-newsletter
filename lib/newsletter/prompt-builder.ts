...
function buildSettingsContext(
  ...
  // Helper to add a setting if it exist
  ...
  // Contact Information
  addSetting("Sener Name", settings.senderName);
  ...
  return `NEWSLETTER SETTINGS:/n${settingLines.join("\n")}\n\n`;
}

/**
 * Builds user instructions setion if provided
 */
...
function buildBodyRequirements(params: NewsletterPromptParams): string[] {
  ...
  if (params.settings?.customFooter) {
    `At the very end, include the required footer content WITHOUT labels like "Footer:" or "Customer Foter:" - just include the content naturally with appropriate formatting (e.g., add a horizontal rule "---" before it)`;
  }
...
function buildImportantNotes(params: NewsletterPromptParams): string[] {
  ...
  if (hasUserInput) {
    notes.push(
      "🔴 CRITICAL: The USER INSTRUCTIONS above are MANDATORY and MUST be incorporated intp the newsletter"
    );
  }
 ...
 * - Expecterd output format
 *
 * @params params - All parameters needed for prompt generation
